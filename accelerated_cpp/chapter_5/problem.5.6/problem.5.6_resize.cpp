...
{
  S_collection fail;
  S_collection::iterator student_iter = students.begin();
  S_collection::iterator pass_iter = students.begin();
  S_collection::size_type new_size = 0;
  while(student_iter != students.end()){
    if(fgrade(*student_iter)){
      // copy the failing student to the fails collection.
      fail.push_back(*student_iter);
    }else{
      // move the passing student to the front of the students collection,
      // while maintaining the order of the passing students.
      *pass_iter = *student_iter;
      ++pass_iter;
      ++new_size;
    }
    ++student_iter;
  }
  // resize the students vector to remove failing students
  students.resize(new_size);
