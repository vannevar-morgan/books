...
//
// (Builds on StringList from Chapter 10, Problem 4)
// List is a doubly linked list that holds values of type Type.
//
template <class Type>
class List{
public:
  ...
  //  typedef ListIterator<value_type>::difference_type difference_type;
  ...
  reference front() {
    if(!m_begin){
      throw std::out_of_range("front() was called on an empty List...");
    }
  ...
  reference back() {
    if(!m_begin){
      throw std::out_of_range("back() was called on an empty List...");
    }
  ...
  void pop_front(){
    if(!m_begin){
      throw std::out_of_range("pop_front() was called on an empty List...");
    }
  ...
  void pop_back(){
    if(!m_begin){
      throw std::out_of_range("pop_back() was called on an empty List...");
    }
