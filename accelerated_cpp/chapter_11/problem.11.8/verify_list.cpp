...
int main(){
  ...
  // test write-access using the non-const version of the List::iterator
  ...
  List<string>::const_iterator c_beg_iter = slist.begin();
  List<string>::const_iterator c_end_iter = slist.end();
  ...
  try{
    List<string>::const_reference front = slist.front();
    cout << "First element in list: " << front << endl;
  }catch(const out_of_range& oor){
    cout << "Out of Range error: " << oor.what() << endl;
  }
  try{
    List<string>::const_reference back = slist.back();
