...
double make_analysis(const std::vector<Student_info>& students, double grade_student(const Student_info& s))
{
  ...
  transform(students.begin(), students.end(), back_inserter(grades), grade_student);
...
{
  out << name << ": median(did) = " << make_analysis(did, analysis) 
      << ", median(didnt) = " << make_analysis(didnt, analysis) << std::endl;
}
